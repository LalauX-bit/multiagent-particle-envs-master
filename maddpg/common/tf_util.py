...
class BatchInput(PlacholderTfInput):
    def __init__(self, shape, dtype=tf.float32, name=None):
        ...
        """
#        super().__init__(tf.placeholder(dtype, [None] + list(shape), name=name))  #replace by LalauX
...
#    tf_config = tf.ConfigProto( #replace by LalauX
    tf_config = tf.compat.v1.ConfigProto(

        inter_op_parallelism_threads=num_cpu,
        intra_op_parallelism_threads=num_cpu)
#    return tf.Session(config=tf_config)  #replace by LalauX
